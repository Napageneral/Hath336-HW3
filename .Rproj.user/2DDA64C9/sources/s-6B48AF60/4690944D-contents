...
#Coordinates
pac_indx = which(cleaned_data[,1]>-60 & cleaned_data[,1]<60)
...
#So we end up with a range of 120 degrees lon and 360 degrees lat
#Since we have a data point every 5 degrees, we have:
#120/5 = 24 values
#360/5 = 72 values
#All these values form a grid, so  we have:
#24x72 = 1728 total values for each date
#We have 50 years of this data, with 12 months a year
#50*12 = 600 dates
#so our final data set is 160 potential coordinates over 600 dates -> 1728x600
...
pacificMeans = matrix(0, nrow = 1728, ncol = 50)
...
#Which is comprised of 1728 values representing 72x24 matrix
#Really no idea why in matrix(U[,1], ncol=24), ncol=24, seems like ncol=72 makes more sense
...
par(mar=c(4,5,3,0))
mapmat=matrix(pacificMeans[,1], nrow=72)
...
filled.contour(Lon, Lat, mapmat, color.palette=rgb.palette, levels=int,
               ...
               plot.title=title(main="Global SST Anomalies [deg C]: Annual 1951",
...
par(mar=c(4,5,3,0))
mapmat=matrix(pacificMeans[,2], nrow=72)
filled.contour(Lon, Lat, mapmat, color.palette=rgb.palette, levels=int,
               xlim=c(0,360),ylim=c(-90,90),
...
par(mar=c(4,5,3,0))
mapmat=matrix(pacificMeans[,3], nrow=72)
filled.contour(Lon, Lat, mapmat, color.palette=rgb.palette, levels=int,
               xlim=c(0,360),ylim=c(-90,90),
